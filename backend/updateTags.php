...
if(isset($postdata) && !empty($postdata))
{
  ...
  if(mysqli_query($con,$sql))
  {
    ...
    $query_check_tags = "SELECT * from tags where `tag` = '$tags'";
    $query_result = mysqli_query($con,$query_check_tags);

    $query = "INSERT INTO tags(tag) VALUES('$tags')";

    if ($query_result->num_rows > 0) {
                         http_response_code(201);
                         echo json_encode(['data'=>'Exist']);
    }else{
